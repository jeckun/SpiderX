...
class Site():
    ...
    def get_articles(self, storys, start, end):
    ...
    def get_article_series(self, story):
        ...
        if story.series[seri]:
            for name, url in story.series[seri].items():
                ...
                if name not in serie_list and name not in story_list:
                    # 关联文章不存在，需要下载
    ...
    def download(self, pages):
        ...
        multi_thread(self.collect_list, get_star_end, pages[1], pages[0]+1, max_lines)
        # thd = []
        # s_e = []
        ...
        # 多线程采集详情页
        multi_thread(self.get_articles, split_list, 0, len(self.storys), max_lines)
        # thd = []
        # s_e = []
