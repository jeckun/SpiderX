...
class Site():
    ...
    def get_article_series(self, story):
        ...
        rst=self.db.query_by_sql("select name from story_download_list;")
    ...
    def save_to_db(self):
        ...
        for story in self.storys:
            ...
            # 插入前检查是否已有记录，有的跳过
            if self.db.query_by_filter('story_download_list',f'name="{story.name}"'):
            ...
            sql = "insert into story_download_list(id, name, url, author, publish, " \
                ...
                + "('%s','%s','%s','%s','%s'," \
                + "'%s','%s','%s','%s');"
            self.db.execute(sql % (id,story.name,story.url,story.author,story.publish, \
