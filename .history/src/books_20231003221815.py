...
class Site():
    ...
    def save_to_db(self):
        ...
        for sto in self.storys:
            id = random_list = random.sample(range(100, 900), 6)
            sql = f"insert into story_download_list(id, name, url, author, publish, " \
            + "category, labels, savepath, series) values " \
            + "('{id}','{sto.name}','{sto.url}','{sto.author}','{sto.publish}'," \
            + "'{sto.category}','','{sto.savepath}','');"
            self.db.execute(sql)
    ...
    def get_story_card(self, page_n):
        ...
        for card in story_cards:
            ...
            st.series = self.check_group(st, self.spider.page.eles(self.config['series']))
    ...
    # 检查是否为系列故事
    def check_group(self, story, series):
        if series:
