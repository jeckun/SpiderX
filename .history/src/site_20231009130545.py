...
class Site():
    ...
    def get_series_list(self, story, tag: str):
        ...
        if series:
            ...
            return {ser_name: dict([(a.name.replace('?','').replace('/','').replace('@','').replace('&','').replace('%',''), a.url) for a in series])} 
        else:
            ser_name = get_group(story.name.replace('?','').replace('/','').replace('@','').replace('&','').replace('%',''), story.name)
            return {ser_name: {}}
    ...
    def download(self, pages, multi=False):
        ...
        if multi:
            ...
            self.check_series()
            multi_thread(self.get_series, split_list, 0, len(self.series), max_lines)
            self.storys += self.series
            self.save_stroys(self.series)
            pass
        else:
            # 获取任务列表
            print('Collect article list...')
            ...
            self.save_stroys(self.storys)

            # 采集关联文章
            print('Collect series article...')
            self.check_series()
            self.get_series(0, len(self.series))
            self.save_stroys(self.series)
            self.storys += self.series
            pass
