...
#             filename, ext = self.check_file_exists(sto)
#             if ext:
#                 pass
#             else:
...
#         # 插入数据
#         print(f"{self.id}#", f'Start save recode.')
#         for sto in [n[1] for n in self.storys if n[0] == self.id]:
...
#                 print(f"{self.id}#", f'recode exists: {sto.name}')
#                 continue
...
#                 + "('%s','%s','%s','%s','%s'," \
#                 + "'%s','%s','%s','%s');"
#             db.execute(sql % (id,sto.name,sto.url,sto.author,sto.publish, \
#                 sto.category,labs,sto.savepath,seri))
#             db.commit()
#             print(f"{self.id}#", f'recode saved to db: {sto.name}')
#         print(f"{self.id}#", f'Recode saved all.')
...
#             # 如果存在，就跳过
#             print(f"{self.id}#", 'file exsist.', os.path.basename(filename))
