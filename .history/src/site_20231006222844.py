...
class Site():
    ...
    def check_list(self):
        ...
        db_list = database.query_by_sql(sql)
        db_name_list = [n[0] for n in db_list]
        for item in self.lists:
            index = db_name_list.index(item)
            if index:
                self.storys.append(Story(name=item[0],url=item[1],savepath=db_list[index][1]))
            else:
