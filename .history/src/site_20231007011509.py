...
from src.lib.base import config, get_star_end, wait, index, get_group
...
class Site():
    ...
    def get_article_series(self, story):
        if story.series:
            seri = story.series
            for item in story.series[seri]:
                filename = os.path.join(BASE_DIR,CONFIG['save_path'],seri,item[0]+'.txt')
                if os.path.exists(filename):
                    continue
                else:
                    # 关联文章不存在，需要下载
                    self.series.append(Story(name=item[0],url=item[1]))
