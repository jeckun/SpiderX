...
class Story:
    ...
    def __str__(self):
        return tostringlist([self.__story_name, self.__story_author, self.__publish_date, self.__category, self.__labels, self.__story_url])
...
class Book():
    ...
    def get_lebals(self, tags=None):
        ...
        for i in range(len(tags)):
            tg[i].append(tags[i].ele('tag:a').text)
        return tgs
