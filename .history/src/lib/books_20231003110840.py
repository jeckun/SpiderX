...
class Story:
    ...
    def __str__(self):
        return '|'.join([self.name, self.author, self.__publish_date, self.__category, ':'.join(self.__labels), self.url])
...
class Book():
    ...
    def download(self, x, y):
        ...
        for ck in self.storys:
            filename=os.path.join(BASE_DIR,self.config['book_path'],ck.name)
