...
class Site():
    ...
    # 开始下载文章
    def download(self, x, y, thid=0):
        self.id = thid

        # 获取下载文章列表
        for p in range(x,y+1):
            print(self.id, "downloading page:", p)
        ...
        for ck in copy.deepcopy(self.storys):
            if ck.series and list(ck.series)[0]:
                ...
                lst=[n.name for n in self.storys]   # 待下载文章列表
        ...
        for i, ck in enumerate(self.storys):
            ...
            if os.path.exists(filename):
                # 如果存在，就跳过
                print(self.id, "%d|%d: " % (i+1, len(self.storys)),'file exsist.', ck.name)
            else:
                # 保存文件
                print(self.id, "%d|%d: " % (i+1, len(self.storys)),'file saved.', ck.name)
    ...
    def save_to_db(self):
        ...
        for sto in self.storys:
            ...
            # 插入前检查是否已有记录，有的跳过
            if self.db.query_by_filter('story_download_list',f'name="{sto.name}"'):
                print(self.id, f'归档过: {sto.name}')
                continue
    ...
    def get_story_card(self, page_n):
        ...
        for card in story_cards:
            ...
            print(self.id, 'Collecting info:', st.name)
