...
class Site():
    ...
    def get_articles(self, star, end):
        for item in storys[start: end]:
    ...
    def download(self, pages):
        ...
        for s, e in get_star_end(pages[0], pages[1], step):
            thd.append(threading.Thread(target=self.collect_list, args=(s , e)))
            thd[i].start()
            time.sleep(1)
            # 防止启动过多线程
        ...
        for s, e in split_list(self.storys, step):
            for i in range(max_lines):
                thd.append(threading.Thread(target=self.get_articles, args=(s, e)))
                thd[i].start()
                time.sleep(1)
                # 防止启动过多线程
                wait(max_linex=max_lines)
